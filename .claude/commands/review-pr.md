...
Use `code-reviewer` agent to analyze for bugs, security, performance, project standards.

Agent returns JSON: `{verdict, summary, issues[], positives[], suggestions[]}`
- Each issue: `{path, line, side, severity, body}`
- Verdict: APPROVE | REQUEST_CHANGES | COMMENT

### 2.3 Parse Agent Response

Extract and validate structured JSON from agent response.
...
Format: Verdict, Summary, Issues table, Positive observations, Non-blocking suggestions.

### 3.3 Prepare Line Comments

Map each issue to GitHub review comment format: `{path, line, side, body}`.
...
Could support `.claude/review-config.json` for autoApprove, ignorePaths, severityThreshold, customChecks.
